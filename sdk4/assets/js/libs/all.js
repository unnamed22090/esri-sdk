...
window.onload = function (){
  ...
  if (window.calcite) {
    ...
    if (document.querySelector("#sample-code-search") || document.querySelector("#sample-code-search-drawer")) {
      ...
      var sampleCodeBaseURLRegEx = /^https?:\/\/[\da-zA-Z\.-]+\.([a-z\.]){2,6}[\/\w\.-]*(?=sample-code)/gm;
        ...
        if (sampleSearchTerm) {
          ...
          if (sampleSearchResults.length > 0) {
            ...
            for (var sampleResult in sampleSearchResults) {
              ...
              var sourceLink = result.sandbox ? "<div class=\"action-bar inline-block\"><a class=\"btn btn-small action-bar-btn phone-trailer-1\" href='" + sandboxURL + result.sample_name + "'>Explore in sandbox</a>" : "<div class=\"action-bar inline-block\">";
              sourceLink += "<a class=\"btn btn-small action-bar-btn\" href='" + preURL + result.live_link + "'>View live</a></div>";
