require([
  ...
  function setSampleFolder() {
    ...
    if (urlObject.query && urlObject.query.sample) {
      ...
      sampleFolder = pathnameWithoutSandbox + "sample-code/get-started-mapview/live";
    }
  }
  ...
  function updateSandbox() {
    ...
    // 4.x fix for syntax used in scene-external-renderer-threejs samples
    //      load("./data/iss.obj"
    var relLinks = content.match(/issMeshUrl = ".+?"/g);
    if (relLinks) {
    ...
    // 4.x fix for sceneview-stars sample
    //      "./Clock.js",
    var relLinks = content.match(/".\/Clock.js"/g);
    if (relLinks) {
      relLinks = relLinks.filter(function(url){ return url.indexOf("//") === -1 }); // ignore absolute URLs
      relLinks.forEach(function(href){
        var file = href.split('"')[1];
    ...
    var relLinks = content.match(/".\/clock.svg"/g);
    if (relLinks) {
      relLinks = relLinks.filter(function(url){ return url.indexOf("//") === -1 }); // ignore absolute URLs
      relLinks.forEach(function(href){
        var file = href.split('"')[1];
    ...
    if (relLinks) {
      relLinks = relLinks.filter(function(url){ return url.indexOf("//") === -1 }); // ignore absolute URLs
      relLinks = relLinks.filter(function(url){ return url.indexOf("{") === -1 }); // ignore {templated-things} as in portalitem-dragndrop      
      relLinks.forEach(function(href){
        var file = href.split('"')[1];
        content = content.replace(file, sampleFolder + '/' + file);
      });
